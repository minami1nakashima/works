<template>
  <v-card
    class="mx-auto border-lg"
    :class="cardSpCheck(table.styleColor)"
    min-height="600"
    flat
  >
    <!-- ランキングタイトル -->
    ...
    <!-- 詳細カード -->
    <v-row>
      <v-col v-for="item in test()" :key="item.id">
        <v-card
          id="relative"
          class="mt-5 mx-auto"
...
<script>
  import GourmetCompDialog from "./GourmetCompDialog.vue";
  ...
  export default {
    ...
    methods: {
      ...
      cardSpCheck(category) {
        return this.$vuetify.display.smAndDown
          ? `pa-1 ${this.table.styleColor}`
          : `pa-5 mb-3 ${this.table.styleColor}`;
      },
      // SP画面区分別処理（datatable背景色）
      tableSpCheck(tcategory) {
        return this.$vuetify.display.smAndDown
          ? `text-caption pa-0 ${this.table.bgColor}`
          : this.table.bgColor;
      },
    },
    computed: {
