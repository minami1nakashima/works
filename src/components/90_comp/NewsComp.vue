  ...
  <v-sheet class="bg-grey-lighten-4 mt-15 pa-6 mx-auto" width="100%">
    <v-row justify="space-between">
      <v-col :cols="SpCheck() ? '5' : '4'" :class="SpCheck() ? '' : 'my-3'">
        <p
          :class="SpCheck() ? 'text-subtitle-2' : 'text-h4'"
          class="font-weight-black newsFont"
        >
          NEWS
        </p>
      </v-col>
    ...
    <v-row>
      <v-col>
        <v-card
          v-if="newsItem"
          class="mx-auto"
          :min-width="SpCheck() ? '300' : '580'"
          :min-height="SpCheck() ? '240' : '335'"
          :class="SpCheck() ? 'pb-0' : ''"
          flat
        >
          <v-img
            :height="SpCheck() ? '150' : '200'"
      ...
      <v-col>
        <v-list
          lines="one"
          class="mx-auto"
          :min-width="SpCheck() ? '300' : '460'"
        >
          <!-- リンク埋め込み -->
          <v-list-item
            v-for="item in newsItemSub"
            :key="item.url"
            density="compact"
            :href="item.url"
            class="mx-3"
          >
            ...
            <v-list-item-title
              v-text="
                momentSub(item) +
                ' ' +
                (japanese ? item.translatedTitle : item.title)
              "
              :class="SpCheck() ? 'text-caption ma-0' : ''"
            ></v-list-item-title>
            <v-divider></v-divider>
          </v-list-item>
        </v-list>
    ...
    methods: {
      ...
      async NewsSub() {
        try {
          for (let i = 0; i <= 7; i++) {
            const response = await axios.get(
              "https://newsapi.org/v2/everything?q=korea&sortBy=popularity&apiKey=98c82fa89a004667b1cc0687d129f86d",
            );
