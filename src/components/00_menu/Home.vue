            ...
            <v-row :class="SpCheck() ? 'mx-1' : 'mx-10'">
                ...
                <v-col v-for="item in homeData.cards" :key="item.theme" class="pa-5">
                    <v-card id="relative" class="mx-auto" :width="SpCheck() ? '' : '240'"
                        ...
                        <!-- クリックでダイアログ表示 -->
                        <!-- 未対応 -->
                        <v-btn id="more" :size="SpCheck() ? 'x-small' : 'small'" class="text-blue font-weight-black"
                            flat :class="SpCheck() ? 'text-caption' : 'text-body-2'"
                            @click="click(item.component)">もっと見る
                        </v-btn>
                    </v-card>
                </v-col>
                ...
                <v-sheet class="bg-grey-lighten-4 mt-15 pa-6 mx-auto">
                    <v-row justify="space-between">
                        ...
                        <v-col :cols="SpCheck() ? '4' : '2'">
                            <v-switch density="compact" label="日本語" v-model="japanese"
                    ...
                    <v-row>
                        <v-col>
                            <v-card v-if="newsItem" class="mx-auto" :width="SpCheck() ? '280' : '580'"
                        ...
                        <v-col :class="SpCheck() ? 'pt-0' : ''">
                            <v-list lines="one" class="mx-auto" :width="SpCheck() ? '280' : '460'">
                                <!-- リンク埋め込み -->
                                <v-list-item v-for="item in newsItemSub" :key="item.url" density="compact"
                                    :href="item.url" class="mx-3">
                                    <!-- ニュースタイトル（日付＋タイトル） -->
                                    <v-list-item-title
...
export default {
    ...
    methods: {
        ...
        // ホテルダイアログ表示処理
        table() {
            let check = this.homeData.cards.filter((value) => {
