<template>
    <v-card class="mx-auto border-lg" :class="cardSpCheck(table.styleColor)" min-height="600" flat>
        ...
        <!-- ランキングサブタイトル -->
        <v-card-text class="text-black" :class="SpCheck() ? 'text-caption pa-0 mb-2' : 'text-body-2'">
            {{ table.sub }}👑
        </v-card-text>
        ...
        <v-row>
            <v-col v-for="item in test()" :key="item.id">
                <v-card id="relative" class="mt-5 mx-auto" :width="SpCheck() ? '150' : '240'"
                    ...
                    <!-- カードタイトル ごはん・カフェ -->
                    <v-card-title v-if="item.page <= '2'" :class="SpCheck() ? 'text-subtitle-2' : ''">
                        {{ item.Place }}
                    </v-card-title>
                    <!-- メニュー・商品名 コンビニ-->
...
<script>
import kwangjangmarket from "@/components/00_Seoul/03_gourmet/dialog/kwangjangmarket.vue";
